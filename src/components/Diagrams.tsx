...
/** Tiny helper for a looped phase timeline without re-rendering every frame */
function usePhases<T extends string>(durations: Record<T, number>): T {
  const keys = Object.keys(durations) as T[];
  const total = keys.reduce((acc, k) => acc + durations[k], 0);
  const [phase, setPhase] = useState<T>(keys[0]);
  const phaseRef = useRef<T>(keys[0]);
  useEffect(() => {
    let raf = 0;
  ...
  }, [total, keys, durations]);
  return phase;
}

/* ===================== ANIMATED DIAGRAMS ===================== */

function TanhaAnimated() {
  // Updated to be accurate: shows quick grab (25-100ms) vs latch zone
  return (
    <svg className="w-full" viewBox="0 0 560 140" role="img" aria-label="Taṇhā grab intensity and latch threshold">
      ...
      {/* Animated grab zone - quick and reversible */}
      <rect x="40" y="60" width="180" height="20" rx="10" fill="#10b981" opacity=".35"
            className="origin-left anim-breath" />
      <rect x="40" y="60" width="180" height="20" rx="10" fill="none" stroke="#10b981" strokeWidth="1.5" strokeDasharray="3 3"/>
      <text x="130" y="55" fill="#10b981" fontSize="11" textAnchor="middle">quick grab (25-100ms)</text>
      ...
      {/* Latch zone - sticky, requires energy to release */}
      ...
      <text x="158" y="20" fill="#10b981" fontSize="11" textAnchor="end">reversible</text>
...
function CVHAnimated() {
  // Compression sweep: ambiguous → specific
  return (
    <svg className="w-full" viewBox="0 0 560 150" role="img" aria-label="CVH compression sweep from ambiguous to specific">
      <defs>
      ...
      <g>
        <path d="M60,40 C120,35 140,35 200,40 L200,110 C140,115 120,115 60,110 Z"
              className="origin-center anim-breath" fill="url(#cvhGrad)" opacity=".6"/>
        ...
        <g className="anim-sweep">
          <rect x="120" y="38" width="24" height="76" fill="#7dd3fc" opacity=".12" />
        </g>
...
function VCHAnimated() {
  // Wave drifts, clamp slides in → wave pauses → clamp releases → drift resumes
  type P = "free" | "in" | "hold" | "out" | "free2";
  const phase = usePhases<P>({ free: 2000, in: 600, hold: 1200, out: 800, free2: 600 });

  const closed = phase === "in" || phase === "hold";
  const paused = phase === "hold";

  return (
    <svg className="w-full" viewBox="0 0 560 150" role="img" aria-label="VCH: clamp freezes local pattern">
      <defs>
      ...
      {/* moving wave */}
      <g className={`origin-left ${paused ? "paused" : "anim-drift"}`} style={{ transform: "translateX(0)" }}>
        <path d="M60,80 C120,40 180,120 240,80 S360,40 420,80" fill="none" stroke="#94a3b8" strokeWidth="2"/>
      </g>

      {/* clamp bars with smooth slide */}
      <g>
        <rect
          x="240" y="40" width="60" height="80"
          fill="#f472b6" opacity=".25"
        />
        <line
          x1="240" y1="40" x2="240" y2="120"
          stroke="#f472b6" strokeWidth="3"
          className="origin-left"
          style={{ transition: "transform .35s ease", transform: closed ? "translateX(8px)" : "translateX(0)" }}
        />
        <line
          x1="300" y1="40" x2="300" y2="120"
          stroke="#f472b6" strokeWidth="3"
          className="origin-left"
          style={{ transition: "transform .35s ease", transform: closed ? "translateX(-8px)" : "translateX(0)" }}
        />
      ...
      {/* Arrow pointing to hold */}
      <line x1="180" y1="30" x2="240" y2="30" stroke="#7dd3fc" strokeWidth="2" markerEnd="url(#arrowVCH)"/>
      <text x="210" y="22" textAnchor="middle" fill="#7dd3fc" fontSize="12" className={closed ? "anim-pulse" : ""}>
        hold
      </text>
    </svg>
  );
...
function LHHAnimated() {
  // Latch-bridge: held plateau with slow detachment rate
  return (
    <svg className="w-full" viewBox="0 0 560 180" role="img" aria-label="Latch-bridge: energy-efficient tension holding">
      <defs>
      ...
      {/* Force curve showing low ATP maintenance */}
      <path d="M40,130 C120,60 180,40 240,40 S360,60 400,100 480,120 520,120"
            fill="none" stroke="#f472b6" strokeWidth="2.5"/>
      
      {/* Latched state band */}
      <rect x="40" y="50" width="480" height="24" fill="url(#gradLB)" className="anim-pulse" style={{ animationDuration: "4s" }} />
      ...
      <text x="360" y="150" fill="#e2e8f0" fontSize="12">can persist minutes→years</text>
      ...
      {/* Moving time marker */}
      <g className="anim-slide-wide" style={{ transform: "translateX(0)" }}>
        <circle cx="40" cy="100" r="5" fill="#7dd3fc" />
      </g>
    </svg>
  );
}

function HemoNeuralAnimated() {
  // Flow → excitability + feedback loop
  return (
    <svg className="w-full" viewBox="0 0 560 160" role="img" aria-label="Hemo-neural coupling with pulsing flow and feedback">
      <defs>
      ...
      {/* Flow line (solid) with arrow */}
      <line x1="240" y1="80" x2="320" y2="80" stroke="#7dd3fc" strokeWidth="2" markerEnd="url(#arrowHN)"/>
      {/* Moving "flow" highlight */}
      <g className="anim-sweep">
        <rect x="240" y="74" width="20" height="12" fill="#7dd3fc" opacity=".25" />
      </g>
      <text x="280" y="70" fill="#7dd3fc" fontSize="12" textAnchor="middle">flow → excitability</text>

      {/* Feedback (dashed, animated dash) */}
      <line x1="320" y1="110" x2="240" y2="110" stroke="#7dd3fc" strokeWidth="1.5"
            strokeDasharray="120" strokeDashoffset="120" className="anim-dash" markerEnd="url(#arrowHN)"/>
      <text x="280" y="124" fill="#7dd3fc" fontSize="12" textAnchor="middle">NO/K⁺/feedback</text>
    </svg>
  );
}

function SOHMsAnimated() {
  // Self-organizing harmonic modes with subtle drift
  return (
    <svg className="w-full" viewBox="0 0 560 160" role="img" aria-label="Self-organizing harmonic modes">
      ...
      <path d="M40,80 C80,40 120,120 160,80 S240,40 280,80 320,120 360,80 400,40 440,80 480,120 520,80"
            fill="none" stroke="#7dd3fc" strokeWidth="2" className="anim-drift"/>
...
function AnnealingAnimated() {
  // Crossfade jagged → smooth landscape showing consolidation
  return (
    <svg className="w-full" viewBox="0 0 560 160" role="img" aria-label="Neural annealing: landscape smoothing over time">
      <defs>
      ...
      <path d="M40,120 C80,60 120,140 160,120 200,100 240,140 280,120 320,100 360,140 400,120 440,100 480,140 520,120"
            fill="none" stroke="#94a3b8" strokeWidth="2" className="anim-crossfade-out"/>
      <path d="M40,120 C90,90 130,120 170,110 210,100 250,115 290,110 330,105 370,112 410,110 450,108 490,112 520,110"
            fill="none" stroke="#10b981" strokeWidth="2" opacity=".9" className="anim-crossfade-in"/>
...
function ActiveInferenceStatic() {
  return (
...
export function ConceptDiagram({ id }: { id: string }) {
  // Animated diagrams
  if (id === "tanha") return <TanhaAnimated />;
  if (id === "sohms") return <SOHMsAnimated />;
  if (id === "hemo-neural") return <HemoNeuralAnimated />;
  if (id === "cvh") return <CVHAnimated />;
  if (id === "vch") return <VCHAnimated />;
  if (id === "latch-bridge" || id === "lhh") return <LHHAnimated />;
  if (id === "annealing") return <AnnealingAnimated />;
  
  // Static diagrams
  if (id === "active-inference") return <ActiveInferenceStatic />;
  if (id === "vasocomputation") return <VasocomputationTriad />;
